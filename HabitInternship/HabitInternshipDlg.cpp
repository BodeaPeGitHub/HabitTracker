...
void CHabitInternshipDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_ADD, suggestionsText);
}
...
//	ON_CBN_SELCHANGE(IDC_COMBO_C, &CHabitInternshipDlg::OnCbnSelchangeComboC)
//ON_CBN_SELCHANGE(IDC_COMBO_C, &CHabitInternshipDlg::OnCbnSelchangeComboC)
...
void CHabitInternshipDlg::loadSugestions(vector<Habit> allSugestions) {
	...
	for (auto sugestion : allSugestions) {
		auto index = suggestions.AddString(CString(sugestion.get_name().c_str()));
